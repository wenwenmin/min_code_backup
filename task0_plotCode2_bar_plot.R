...
year_list <- c(1952, 2007)
# save subset of gapminder as data_graph 
data_graph <- gapminder %>% filter(country %in% country_list) %>% filter(year %in% year_list)
data_graph <- data_graph %>% mutate(year = factor(year))
